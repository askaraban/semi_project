...
<%@page import="xzy.itwill.DAO.ReviewDAO"%><!-- qa dao -->
<%@page import="xyz.itwill.DTO.ReviewDTO"%><!-- qa dao -->
...
<%
	...
	int totalPage=(int)Math.ceil((double)qaReview/pageSize);//페이지의 총갯수
	if(totalPage == 0) {
		totalPage=1;
	}
...
/* 						String url=request.getContextPath()+"/main_page/main.jsp?group=qa_page&worker=qa_detail"
				...
				<td>
					...
					<% if(currentDate.equals(qa.getQaRegister().substring(0, 10))) { %>
						<%=qa.getQaRegister().substring(0, 10) %>
					<% } else { %>
						<%=qa.getQaRegister().substring(0, 10) %>
					<% } %>
				</td>		
