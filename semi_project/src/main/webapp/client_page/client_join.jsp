...
<%-- => [아이디 중복 검사] 태그를 클릭한 경우 새창을 열어 [client_page/id_check.jsp] 문서를 요청 - 아이디 전달 --%>
<%-- => [우편번호 검색] 태그를 클릭한 경우 [다음 우편번호 서비스]를 사용하여 입력태그(우편번호와 기본주소)의 입력값 변경 --%>    
<%-- => [회원가입] 태그를 클릭한 경우 [/client_page/client_join_action.jsp] 문서를 요청하여 페이지 이동 - 입력값 전달 --%>    
<style type="text/css">
fieldset {
	text-align: left;
	margin: 10px auto;
...
<form id="join" action="<%=request.getContextPath() %>/main_page/main.jsp?group=client_page&worker=client_join_action" method="post">
...
$("#idCheck").click(function() {
	...
	window.open("<%=request.getContextPath()%>/client_page/id_check.jsp?id="+$("#id").val()
