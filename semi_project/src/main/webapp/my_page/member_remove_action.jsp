...
<%
	...
	if(request.getParameter("clientPasswd")!=null) {
		String passwd=Utility.encrypt(request.getParameter("clientPasswd"));
	*/
	...
	clientTable.setClientNum(loginClient.getClientNum());
	clientTable.setClientStatus(0);//회원정보의 회원상태를 [0]으로 변경 - 탈퇴회원
	
	//회원정보를 전달받아 CLIENT_TABLE 테이블에 저장된 회원정보의 회원상태를 변경하고 변경행의 갯수를
	//반환하는 ClientDAO 클래스의 메소드 호출
	ClientDAO.getDAO().updateClientStatus(clientTable);
