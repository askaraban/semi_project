			...
			<li id="anchorBoxId_1" class="xans">
				<div class="card border-light" style="width: 14rem;">
					...
					<div class="card-body item-box">
						...
						<p>
						<%if(isLike) {%> <img src="<%=request.getContextPath()%>/images/icon/heart-red.png" class="wishHeart" alt="좋아요" class="likee" onclick="like()" id="productNum<%=pro.getProductNum()%>">
						<%} else {%> <img src="<%=request.getContextPath()%>/images/icon/heart-black.png" class="wishHeart" alt="좋아요" class="noLikee" onclick="like()"><%} %>
						</p>
...
btn.addEventListener('click',function(){
          ...
          var productNum = $(".item-box").filter("img:nth-child(2)").attr("id");
          alert(productNum);
  })
...
function like() {
	...
	if(isHeart){
		for(i=0;i<classLike.length;i++){
			classList.getItem(i).setAttribute('src', "<%=request.getContextPath()%>/images/icon/heart-black.png");
			classList.getItem(i).setAttribute('title','off');
		}
	} else {
		for(i=0;i<classLike.length;i++){
			classList.getItem(i).setAttribute('src', "<%=request.getContextPath()%>/images/icon/heart-red.png");
			classList.getItem(i).setAttribute('title','on');
		}
	}	
